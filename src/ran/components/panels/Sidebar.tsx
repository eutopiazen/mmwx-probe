...
export function Sidebar({
  ...
  const crossPageBase = crossPage ? `./${currentSearch}` : ''
  ...
  return (
    <>
      ...
      <aside style={asideStyle}>
        ...
        >
          <a
            href={crossPage ? `${crossPageBase}#/overview` : hashFor({ name: 'overview' })}
            title="返回首页"
            style={{
              display: 'flex',
              alignItems: 'center',
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', padding: 8, gap: 1 }}>
          {nav.map((item) => {
            ...
            const href =
              item.id === 'map'
                ? `./map.html${currentSearch}`
                : item.id === 'hub' && item.uuidLink
                  ? crossPageBase + hashFor({ name: 'hub', uuid: item.uuidLink })
                  : crossPageBase + hashFor({ name: item.id } as Route)
        ...
        <div style={{ marginTop: 'auto', padding: 12, borderTop: '1px solid var(--edge-engrave)' }}>
          ...
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
              ...
              <span style={{ fontWeight: 600 }}>RAN · OVERVIEW</span>
            </span>
            <span
              style={{
                fontSize: 11,
