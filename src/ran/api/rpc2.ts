...
export function supportsMetricStore(): Promise<boolean> {
  // mmwx-probe exposes its own history endpoint rather than Komari RPC2.
  // Keep the complete Ran history pipeline intact, but route it through the
  // legacy adapter in client.ts instead of issuing noisy /api/rpc2 probes.
  metricStoreSupport ??= Promise.resolve(false)
