...
const API_CITY_COORDS: Record<string, [number, number]> = {
...
export function nodeToLonLat(node: KomariNode): [number, number] | undefined {
  const apiCity = API_CITY_COORDS[normalizedCity(node.region_city)]
...
export function nodeToCityLabel(node: KomariNode): string | undefined {
  if (node.region_city?.trim()) return node.region_city.trim()
  const name = node.name ?? ''
  if (name) {
